...
class PostgresRowSnapshotCodecTest {
    private final PostgresRowSnapshotCodec codec = new PostgresRowSnapshotCodec(new SnapshotDataCodec(CompressorRegistry.DEFLATE)); // 用同一个解码器读取各压缩方式生成的快照, 验证读取结果与写入时的压缩配置无关
    ...
    void largeRawDataRoundTripsAndOnlyEncodedFrameSizeDependsOnCompression(CompressorRegistry compressor) throws IOException {
        var codec = new PostgresRowSnapshotCodec(new SnapshotDataCodec(compressor));
    ...
    void everyCompressorCanBeReadByTheSameDecoder(CompressorRegistry compressor) throws IOException {
        ...
        SnapshotRow row = new PostgresRowSnapshotCodec(new SnapshotDataCodec(compressor)).encode(snapshot);
