...
final class PostgresSchemaMigrator {
    ...
    PostgresSchemaMigrator(@NotNull SyncLogger logger, int currentVersion, @NotNull BiConsumer<Handle, String> initializer, @NotNull List<PostgresSchemaMigration> migrations) {
        this.logger = logger;
        ...
        try {
            jdbi.useHandle(handle -> {
                ...
                handle.useTransaction(transaction -> {
                    ...
                    transaction.createQuery("SELECT pg_advisory_xact_lock(1936946030, hashtext(current_schema() || ':' || :prefix))")
    ...
    private void migrateLocked(Handle handle, String prefix) {
        ...
        if (stored == 0) {
            long existing = handle.createQuery("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name IN (<tables>)")
                    .bindList("tables", prefix + "snapshots", prefix + "users", prefix + "maps").mapTo(Long.class).one();
            if (existing != 0) {
                throw new IllegalStateException("PostgreSQL business tables exist without a schema version");
            }
            this.logger.info(LogCategory.STORAGE, LogConstants.STORAGE_POSTGRESQL_SCHEMA_INITIALIZING, prefix, String.valueOf(this.currentVersion));
        ...
        for (int i = (int) stored - 1; i < this.migrations.size(); i++) {
            ...
            this.logger.info(LogCategory.STORAGE, LogConstants.STORAGE_POSTGRESQL_SCHEMA_MIGRATING, prefix, String.valueOf(target - 1), String.valueOf(target));
