...
public final class TestCommand extends BukkitCommandFeature {
    ...
    private void smokeTest(CommandContext<CommandSender> context) {
        ...
        Player target = context.<Player>optional("player").orElse(sender instanceof Player self ? self : null);
        if (target == null) {
            sender.sendMessage(Component.text("console must specify a player: /sparrow-sync test smoke <player>", NamedTextColor.RED));
            return;
        }
