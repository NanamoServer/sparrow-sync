...
final class MysqlSchemaMigrator {
     ...
     */
    MysqlSchemaMigrator(int currentVersion, @NotNull BiConsumer<Handle, String> initializer, @NotNull List<MysqlSchemaMigration> migrations) {
    ...
    void migrate(@NotNull Jdbi jdbi, @NotNull String prefix) {
        ...
        jdbi.useHandle(handle -> {
            ...
            String lock = handle.createQuery("SELECT SHA2(CONCAT('sparrow-sync-schema:', DATABASE(), ':', :prefix), 256)").bind("prefix", prefix).mapTo(String.class).one();
            Integer acquired = handle.createQuery("SELECT GET_LOCK(:name, 5)").bind("name", lock).mapTo(Integer.class).one();
            if (!Integer.valueOf(1).equals(acquired)) throw new IllegalStateException("Could not acquire the MySQL schema migration lock");
            try {
