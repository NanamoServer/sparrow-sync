...
public abstract class AbstractJavaScheduler implements SchedulerAdapter {
    ...
    public AbstractJavaScheduler(Plugin plugin) {
        ...
        this.scheduler = new ScheduledThreadPoolExecutor(4, r -> {
            ...
            thread.setName("sparrow-sync-scheduler");
            return thread;
        });
    ...
    public void shutdownScheduler() {
        ...
        try {
            if (!this.scheduler.awaitTermination(1, TimeUnit.MINUTES)) {
                ...
                reportRunningTasks(thread -> thread.getName().equals("sparrow-sync-scheduler"));
            }
    ...
    public void shutdownExecutor() {
        ...
        try {
            if (!this.worker.awaitTermination(1, TimeUnit.MINUTES)) {
                ...
                reportRunningTasks(thread -> thread.getName().startsWith("sparrow-sync-worker-"));
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
    ...
    private static final class WorkerThreadFactory implements ForkJoinPool.ForkJoinWorkerThreadFactory {
        ...
        public ForkJoinWorkerThread newThread(ForkJoinPool pool) {
            ...
            thread.setName("sparrow-sync-worker-" + COUNT.getAndIncrement());
            return thread;
        }
    }
