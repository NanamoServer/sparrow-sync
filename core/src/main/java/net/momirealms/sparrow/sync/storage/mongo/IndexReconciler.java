...
final class IndexReconciler {
    ...
    private static final List<IndexDeclaration> MAP_INDEXES = List.of(
            new IndexDeclaration(new Document("owner", 1).append("origin_id", 1), true, "map_source"),
            new IndexDeclaration(new Document("updated_at", 1), false, "map_updated_at")
    );
    ...
    static void reconcile(
        ...
        // 版本检查通过后再整理全部业务集合, 地图索引失败时保留原完成版本.
        reconcileIndexes(logger, database, database.getCollection(prefix + "users"), USER_INDEXES);
        reconcileIndexes(logger, database, database.getCollection(prefix + "snapshots"), SNAPSHOT_INDEXES);
        reconcileIndexes(logger, database, database.getCollection(prefix + "maps"), MAP_INDEXES);
    ...
    private static void reconcileIndexes(SyncLogger logger, MongoDatabase database, MongoCollection<Document> collection, List<IndexDeclaration> declarations) {
